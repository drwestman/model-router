...
let _cachedDelegationProtocolTemplate = null;
...
function getDelegationProtocolTemplate() {
    if (_cachedDelegationProtocolTemplate === null) {
        try {
            _cachedDelegationProtocolTemplate = readFileSync(delegationProtocolTemplatePath(), "utf-8");
        }
        catch {
            console.warn("[model-router] delegation protocol template unavailable; continuing without it.");
...
export function resolveRouterPaths(env = process.env) {
    const configuredConfigPath = normalizeConfiguredPath(env.OPENCODE_MODEL_ROUTER_CONFIG_PATH);
    const configuredStatePath = normalizeConfiguredPath(env.OPENCODE_MODEL_ROUTER_STATE_PATH);
    ...
    const statePath = configuredStatePath ||
        join(homedir(), ".config", "opencode", "opencode-model-router.state.json");
    if (env.MODEL_ROUTER_DEBUG === "1") {
        console.warn(`[model-router] resolveRouterPaths configPath=${configPath} statePath=${statePath}`);
    }
    return {
        configPath,
        statePath,
    ...
    for (const [presetName, preset] of Object.entries(presets)) {
        ...
        for (const [tierName, tier] of Object.entries(tiers)) {
            ...
            if (!isStringArray(t.whenToUse)) {
                throw new Error(`tiers.json: '${presetName}.${tierName}.whenToUse' must be an array of strings`);
            }
        }
    }
    ...
    if (obj.modes !== undefined) {
        ...
        for (const [modeName, mode] of Object.entries(modes)) {
            ...
            if (m.overrideRules !== undefined && !isStringArray(m.overrideRules)) {
                throw new Error(`tiers.json: mode '${modeName}.overrideRules' must be an array of strings`);
            }
            if (m.tierPrompts !== undefined) {
                if (typeof m.tierPrompts !== "object" ||
                    m.tierPrompts === null ||
                    Array.isArray(m.tierPrompts)) {
                    throw new Error(`tiers.json: mode '${modeName}.tierPrompts' must be an object`);
                }
                for (const [tierName, prompt] of Object.entries(m.tierPrompts)) {
                    if (typeof prompt !== "string") {
                        throw new Error(`tiers.json: mode '${modeName}.tierPrompts.${tierName}' must be a string`);
                    }
                }
            }
        }
...
export function applyPersistedState(cfg, state) {
...
export function normalizeRouterState(raw) {
...
export function writeStateFile(filePath, patch) {
    const state = normalizeRouterState({ ...readStateFile(filePath), ...patch });
    mkdirSync(dirname(filePath), { recursive: true });
    writeFileSync(filePath, JSON.stringify(state, null, 2) + "\n", "utf-8");
}
export function loadConfigFromPaths(paths = resolveRouterPaths()) {
...
function safeLoadConfig() {
    try {
        ...
        console.warn(`[model-router] using fallback config after load failure: ${error instanceof Error ? error.message : String(error)}`);
...
function loadStartupConfig() {
    ...
    try {
        ...
        console.warn(`[model-router] failed to resolve startup paths: ${error instanceof Error ? error.message : String(error)}`);
        paths = {
...
function warnIfProviderMetadataMissing(opencodeConfig, cfg, tiers) {
    ...
    const activeProvider = typeof firstTier?.model === "string"
    ...
    const enabledProviders = Array.isArray(opencodeConfig.enabled_providers)
        ...
        : [];
    const providerAvailable = Boolean(providerConfig[activeProvider]) ||
        enabledProviders.includes(activeProvider);
    if (!providerAvailable) {
        console.warn(`[model-router] provider metadata for '${activeProvider}' is unavailable during config(); router agents still registered for preset '${getActivePresetName(cfg)}'`);
    }
}
// ---------------------------------------------------------------------------
...
function getActiveTiers(cfg) {
    ...
    const activePreset = typeof cfg.activePreset === "string" && presets[cfg.activePreset]
        ? cfg.activePreset
...
export function composePrompt(prefix, prompt) {
...
function buildPonytailPrompt(tierName, cfg) {
    if (cfg.activeMode !== PONYTAIL_MODE_NAME || !tierName)
        return undefined;
    if (!PONYTAIL_ENABLED_TIERS.has(tierName))
        return undefined;
...
function buildFallbackInstructions(cfg) {
    ...
    const chains = Object.entries(map ?? {}).flatMap(([provider, presetOrder]) => {
        if (!Array.isArray(presetOrder))
            return [];
        const valid = presetOrder.filter((p) => p !== activePreset && Boolean(cfg.presets[p]));
...
export function buildBudgetOutput(cfg, args) {
...
// ---------------------------------------------------------------------------
export function buildPresetOutput(cfg, args) {
...
export function extractDispatchText(output) {
    const o = output;
...
export function buildCapBanner(state, isRedundant, previousCall, tool) {
    ...
    if (isRedundant && previousCall !== undefined) {
        lines.push(`[⚠ REDUNDANT: this is the same ${tool} you ran at call #${previousCall}. STOP now — repeated reads add no information. Return with DONE/NEED MORE/NEED CONTEXT/SCOPE GROWTH/ESCALATE.]`);
    }
    if (state.cap !== "none") {
        const remaining = state.cap - state.calls;
        if (remaining <= 0) {
            lines.push(`[⚠ CAP REACHED (${state.calls}/${state.cap}): your NEXT response MUST be a return — do NOT make another read-only call. Start the response with DONE:, NEED MORE:, NEED CONTEXT:, SCOPE GROWTH:, or ESCALATE:.]`);
        }
        else if (remaining <= 2) {
...
const ModelRouterPlugin = (_ctx) => {
    ...
    const subagentCapState = new Map();
    ...
    return {
        ...
        "tool.execute.after": async (input, output) => {
            ...
            if (!state)
                return; // not a tracked subagent session
        ...
        "experimental.text.complete": async (input, output) => {
            if (bypassed)
                return;
            const safeOutput = typeof output === "object" && output !== null ? output : undefined;
        ...
        "experimental.chat.system.transform": async (_input, output) => {
            ...
            if (sessionID && subagentSessionIDs.has(sessionID)) {
                const ponytailPrompt = buildPonytailPrompt(subagentCapState.get(sessionID)?.tierName, cfg);
                if (ponytailPrompt) {
            ...
            const providerPrefix = resolveProviderAdapter(orchestratorModel).buildOrchestratorPromptPrefix(orchestratorModel);
...
export function buildAgentDefinition(name, tier, cfg) {
    ...
    const providerPromptPrefix = providerAdapter.buildTierPromptPrefix(name, tier.model);
...
export function registerActiveTierAgents(opencodeConfig, cfg, tiers = getActiveTiers(cfg)) {
    opencodeConfig.agent ??= {};
    for (const [name, tier] of Object.entries(tiers ?? {})) {
        if (typeof tier !== "object" || tier === null) {
            console.warn(`[model-router] skipping invalid tier '${name}' in active preset '${getActivePresetName(cfg)}'`);
            continue;
        }
