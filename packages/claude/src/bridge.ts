...
import {
  ...
  packageRootFrom,
  readConfig,
  readState as readPersistedState,
  resolveModeName,
  resolvePolicy,
  resolvePresetName,
  type RouterPaths,
...
type Confidence = "ambiguous" | "moderate" | "very-high";

export interface ClaudeRouterState extends RouterState {
  bypass?: boolean;
}
...
const RULE_KEYWORDS: Record<TierName, string[]> = {
  fast: [
    "search",
    "inspect",
    "read",
...
const COMMAND_USAGE =
  "run-command <tiers|preset|mode|bypass|delegate|annotate-plan|ponytail-review> [args]";
...
function resolveAdapterPaths(env: NodeJS.ProcessEnv = process.env): RouterPaths {
...
function getPolicyDefaultTier(policyDefaultTier: string): TierName {
  return isTierName(policyDefaultTier) ? policyDefaultTier : "medium";
}

function scorePrompt(
  config: RouterConfig,
  state: ClaudeRouterState,
  prompt: string,
): PromptClassification {
  const text = normalizeText(prompt).toLowerCase();
  const policy = resolvePolicy(config, state);
  const scores: Record<TierName, number> = { fast: 0, medium: 0, heavy: 0 };
  let strongTier: TierName | undefined;
...
function getActivePreset(config: RouterConfig, state: ClaudeRouterState): Record<string, TierConfig> {
  const policy = resolvePolicy(config, state);
  return config.presets[policy.activePreset] ?? {};
}

function getTierContract(tierName: TierName): string {
  switch (tierName) {
    case "fast":
...
function buildDelegationTemplate(
  config: RouterConfig,
  state: ClaudeRouterState,
  tierName: TierName,
  task: string,
...
function buildRoutingHint(classification: PromptClassification): string | null {
...
function buildPromptContext(
  ...
  const preset = getActivePreset(config, state);
...
function readPromptFromStdin(): string | null {
  let input: string;
  ...
  try {
    const parsed = JSON.parse(input) as { prompt?: unknown; data?: { prompt?: unknown } };
...
function emit(text: string | null, event = "UserPromptSubmit"): void {
  if (!text) {
    return;
  }
...
function listPresetNames(config: RouterConfig): string[] {
  return Object.keys(config.presets ?? {});
}

function listGlobalModeNames(config: RouterConfig): string[] {
  return Object.keys(config.modes ?? {});
}

function defaultModeForPreset(config: RouterConfig): string | undefined {
  return config.activeMode ?? listGlobalModeNames(config)[0];
}

function getPreset(config: RouterConfig, presetName: string): Record<string, TierConfig> | null {
  const resolved = resolvePresetName(config, presetName);
  return resolved ? config.presets[resolved] ?? null : null;
}

function getMode(config: RouterConfig, modeName: string): ModeConfig | null {
  const resolved = resolveModeName(config, modeName);
  return resolved ? config.modes?.[resolved] ?? null : null;
}

function getTier(config: RouterConfig, presetName: string, tierName: string): TierConfig | null {
...
function defaultState(config: RouterConfig): ClaudeRouterState {
  return {
...
function normalizeState(config: RouterConfig, input: unknown): ClaudeRouterState {
  ...
  const value = input as ClaudeRouterState;
  const resolvedPreset =
    typeof value.activePreset === "string"
...
function loadState(config: RouterConfig): ClaudeRouterState {
  return normalizeState(config, readPersistedState<ClaudeRouterState>(resolveAdapterPaths().statePath));
}

function saveState(config: RouterConfig, input: unknown): ClaudeRouterState {
  const next = normalizeState(config, input);
  writePersistedState(resolveAdapterPaths().statePath, next);
  return next;
}

function statusLine(state: ClaudeRouterState): string {
  return `active: ${state.activePreset}/${state.activeMode} | bypass: ${state.bypass ? "on" : "off"}`;
}

function buildSessionText(config: RouterConfig, state: ClaudeRouterState): string {
  const normalizedState = normalizeState(config, state);
...
function buildTiersText(config: RouterConfig, state: ClaudeRouterState): string {
  const normalizedState = normalizeState(config, state);
  const policy = resolvePolicy(config, normalizedState);
...
function buildStateText(state: ClaudeRouterState): string {
  return `[model-router] ${statusLine(state)}`;
}

function usage(command: string, values: string): string {
  return `[model-router] usage: ${command} ${values}`;
}

function getPlanSteps(text: string): string[] {
...
function annotateStep(config: RouterConfig, state: ClaudeRouterState, step: string): string | null {
  ...
  const tier =
    classification.confidence === "ambiguous"
      ? getPolicyDefaultTier(resolvePolicy(config, state).defaultTier)
      : classification.tier;
  return `${prefix}${body} [tier:${tier}]`;
}

function handlePreset(config: RouterConfig, state: ClaudeRouterState, args: string[]): string {
...
function handleMode(config: RouterConfig, state: ClaudeRouterState, args: string[]): string {
...
function handleBypass(config: RouterConfig, state: ClaudeRouterState, args: string[]): string {
  const value = args[0];
...
function handleDelegate(config: RouterConfig, state: ClaudeRouterState, argText: string): string {
  ...
  if (!getActivePreset(config, normalizedState)[tierName]) {
...
function handleAnnotatePlan(config: RouterConfig, state: ClaudeRouterState, argText: string): string {
...
function handlePonytailReview(argText: string): string {
...
function normalizeCommandName(command: unknown): string | null {
...
function promptSubmitMain(): void {
...
function runCommandMain(argv = process.argv.slice(2)): void {
